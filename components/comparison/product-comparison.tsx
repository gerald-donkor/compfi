...
export function ProductComparison({ comparison, className, ...props }: ProductComparisonProps) {
  ...
  return <section {...props} className={cn("product-comparison", className)} data-slot="product-comparison">
    <Container>
      ...
      {comparison.count ? <><div className="product-comparison__summaries">{comparison.products.map((product) => <ComparisonProductSummary key={product.id} product={product} removeHref={comparison.removeHref(product.slug)} />)}
